...
Blockly.JSON.blockToObject_ = function(block) {
  ...
  function titleToObject(title) {
    //maybe multiple???
    element[Blockly.JSON.fieldLabels.title] = element[Blockly.JSON.fieldLabels.title] ||[];
  ...
  for (var i = 0, input; input = block.inputList[i]; i++) {
    ...
    if (!empty) {
      element[Blockly.JSON.fieldLabels.inputList] =
        element[Blockly.JSON.fieldLabels.inputList] || [];
  ...
  if (block.nextConnection) {
    ...
    if (nextBlock) {
      element[Blockly.JSON.fieldLabels.nextBlock] =
        element[Blockly.JSON.fieldLabels.nextBlock] || [];
...
Blockly.JSON.objectToBlock_ = function(workspace, jsonBlock) {
  ...
  if (jsonBlock[Blockly.JSON.fieldLabels.comment]){
    block.setCommentText(
        jsonBlock[Blockly.JSON.fieldLabels.comment][Blockly.JSON.fieldLabels.commentText]
    );
    var visible =
      jsonBlock[Blockly.JSON.fieldLabels.comment][Blockly.JSON.fieldLabels.commentPinned];
    if (visible) {
      block.comment.setVisible(true);
    }
    var bubbleW =
      jsonBlock[Blockly.JSON.fieldLabels.comment][Blockly.JSON.fieldLabels.commentWidth];
    var bubbleH =
      jsonBlock[Blockly.JSON.fieldLabels.comment][Blockly.JSON.fieldLabels.commentHeight];
  ...
  if (jsonBlock[Blockly.JSON.fieldLabels.inputList]){
    for (var i=0,jsonInput; jsonInput = jsonBlock[Blockly.JSON.fieldLabels.inputList][i];i++){
      ...
      if (jsonInput[Blockly.JSON.fieldLabels.childBlocks]) {
        var blockChild = Blockly.JSON.objectToBlock_(workspace,
            jsonInput[Blockly.JSON.fieldLabels.childBlocks]
        );
